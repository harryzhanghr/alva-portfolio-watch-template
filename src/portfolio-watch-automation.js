...
function buildAnalystPrompt(input) {
  ...
  return [
    ...
    "- Use not_qualified for any candidate you do not promote into an eventImpactFinding, including stale, routine, weak, repeated, or semantically wrong rows. Use suppressed only when you created a linked eventImpactFinding but decided it should not be selected for the notification.",
	    ...
	    "- Event lane judges thesis, risk, watchpoint, and portfolio exposure impact, not price-move attribution. A direct company event or source-grounded related-holding event can become an eventImpactFinding even when it does not explain the same-session asset anomaly.",
	    "- Judge each directly relevant event on its own materiality. Current price/volume or anomaly context may change urgency and framing, but it is not required for event selection and should not be used as the event's qualification test.",
    ...
    "- For indexed-X breaking_news records, publishedAtMs/sourceTimeLabel should reflect the X post freshness when Pi returned it, while metadata.sourceEventTime/sourceEventAtMs is the original/official or earliest credible source time. Compare them; if the X post is fresh but sourceEventTime is old, treat it as resurfaced/newly discussed unless there is clear new information or new market reaction.",
	    "- Do not push routine price-target noise, broad market color with no portfolio implication, semantically wrong/stale event candidates, or unsourced causal narratives.",
	    "",
	    "5. Data contract",
    ...
    "- For multiple selected findings, use real newlines and '- ' bullets. Use one bullet per selected event_impact or anomaly_attribution finding. Each bullet should be self-contained: event/anomaly, insight, portfolio relevance, thesis/risk read, watchpoint, and, when useful source evidence supports the stated read, one source link.",
    ...
    "- For each selected event_impact or anomaly_attribution finding, attach one inline Markdown link only when a source URL, supporting_events[].url, or sourceLinks[] materially supports the sourced fact, catalyst, attribution, or useful counterpoint. For anomaly_attribution, choose the source that best supports the stated read. If the read is no fresh catalyst, no clean catalyst, unexplained, or weak_correlation and available links are only stale, routine, or background context, use no link. For a merged finding, link the source that best supports the main read; use a second link only when two independent facts are both central. Anchor text should be short and specific, preferably the publisher or source name. Never use generic anchor text such as source, link, article, here, click, or more.",
