...
- The production implementation has two Alva Ask (LLM) call types through `@alva/alvaask`:
  ...
  - Final analyst gate: code calls `ask(buildAnalystPrompt(analystInput))` whenever event-impact candidates or computed asset anomalies exist. First run is context in `portfolio_context.current_portfolio_delta.firstRun`, not an automatic skip. The final analyst now acts as a low-noise PM note generator: selected findings include `decision_lens`, and `notification_message` chooses a compact single-finding note or one bullet per finding for multiple selected findings. When present, `portfolio_watch_preferences_note` is treated as the user's analyst instruction plugin for attention priorities and framing. Each selected event finding carries a short specific source link when available, with explicit thesis/risk, key levels, and watch-next.
...
- The analyst packet is intentionally bounded: recent event records are capped
  at 100 records, review candidates are capped at 100 items, recent alert
  history is included, persisted finding history is not sent to the analyst,
  optional preferences are capped at 30,000 characters, and the compact prompt JSON is capped at 1,000,000 characters. The Pi
