...
- The production implementation has three Alva Ask (LLM) call types through `@alva/alvaask`:
  ...
  - Final analyst gate: code calls `ask(buildAnalystPrompt(analystInput))` whenever event-impact candidates or computed asset anomalies exist. First run is context in `portfolio_context.current_portfolio_delta.firstRun`, not an automatic skip. The final analyst now acts as a low-noise PM note generator: selected findings include `decision_lens`, and `notification_message` is required to be multi-line, chat-readable, short-link anchored, and explicit on notable context, thesis/risk, key levels, and watch-next.
