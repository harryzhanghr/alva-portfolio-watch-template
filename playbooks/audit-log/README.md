...
Current runtime note: portfolio ingest is code-side source access, not an Alva Ask call. Dynamic mode reads one or more connected-account snapshots each run and aggregates holdings/cash into one portfolio before analysis; static mode reads the configured ALFS portfolio file each run. The run log keeps this under `llmDecisionJson.portfolioReader` for backwards compatibility with older audit rows, and future rows should show `environment: Code`, `portfolioMode`, `positionCompleteness`, configured account ids, aggregate account label, and the source call used. Pi Agent handles current theme extraction and external breaking-news portfolio mapping review. Alva Ask is expected for per-asset anomaly attribution agents when anomalies exist and the final analyst gate when candidates or anomalies require review. When configured, `portfolioWatchPreferencesJson` shows analyst-only preference file metadata such as loaded state, path, char count, truncation flag, and hash.
...
Future rows also write compact `rawEventsJson`, `eventCandidatesJson`, `qualifiedEventsJson`, `selectedEventsJson`, `anomaliesJson`, `anomalyAttributionPacketsJson`, `anomalyAttributionsJson`, `finalStatusesJson`, `searchExpansionTraceJson`, and `portfolioWatchPreferencesJson` artifacts into both `analysis.decision` and `audit.run_log`. The Run Logs view therefore shows the exact normalized raw events used to build candidates, the long list, analyst-qualified event assessments, computed anomalies, per-asset anomaly attribution packets, final anomaly attributions, external feed read and mapping-review traces, preference-file metadata, and the final status/reason for every item without relying on large legacy JSON blobs. `selectedEventsJson` remains an internal/compatibility subset of qualified event findings used by the final notification decision; it is not displayed as a separate event-lane stage. Sent alerts should read as short PM notes: compact 2-3 sentence notes for single selected findings, or one bullet per finding when multiple findings are selected, with per-event source links when available and thesis/risk/key-level/watch-next framing. Historical rows that only have older `event.items` / `candidateSummaryJson` / `candidateAuditJson` fields are recovered best-effort when possible.
